...
import { Table, Tag, Typography, Button, Space, Avatar, Tooltip, Input } from 'antd';
import { EyeOutlined, UndoOutlined, UserOutlined, SearchOutlined } from '@ant-design/icons';

const { Text } = Typography;

const PaymentTable = ({ loading, payments, pagination, onChange, onRefund, onViewDetail }) => {
  ...
  const getAvatarColor = (name) => {
    const colors = ['#f56a00', '#7265e6', '#ffbf00', '#00a2ae', '#16a34a', '#3b82f6'];
    let hash = 0;
    if (name) {
  ...
  const columns = [
      ...
      render: (_, record) => (
        <Space size="small">
          ...
          <Tooltip title="Hoàn tiền">
            <Button 
              type="text" 
              icon={<UndoOutlined style={{ color: record.paymentStatus === 'SUCCESS' ? '#f59e0b' : '#cbd5e1' }} />} 
    ...
    {
      title: <Text style={{ fontSize: '12px', color: '#64748b', fontWeight: 600 }}>MÃ GD</Text>,
      dataIndex: 'transactionCode',
      key: 'transactionCode',
      ...getColumnSearchProps('transactionCode', 'mã GD'),
      render: (text) => <Text style={{ color: '#1e293b', fontWeight: 600 }}>{text || 'N/A'}</Text>,
    },
    {
      ...
      render: (_, record) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <Avatar 
            style={{ backgroundColor: getAvatarColor(record.studentName), borderRadius: '6px' }}
            icon={!record.studentName && <UserOutlined />}
            size={36}
          >
            {record.studentName ? record.studentName.charAt(0).toUpperCase() : ''}
          </Avatar>
          <Text style={{ fontWeight: 600, color: '#1e293b' }}>{record.studentName || 'Chưa cập nhật'}</Text>
        </div>
      ),
    },
    {
      ...
      render: (text) => <Text style={{ color: '#475569' }}>{text?.length > 30 ? text.substring(0, 30) + '...' : text}</Text>
    },
    {
      title: <Text style={{ fontSize: '12px', color: '#64748b', fontWeight: 600 }}>SỐ TIỀN (VNĐ)</Text>,
      dataIndex: 'amount',
      key: 'amount',
    ...
    {
      title: <Text style={{ fontSize: '12px', color: '#64748b', fontWeight: 600 }}>PHƯƠNG THỨC</Text>,
      ...
      onFilter: (value, record) => record.paymentMethod === value || (record.paymentMethod || '').toUpperCase().includes(value),
      render: (text) => <Text style={{ color: '#64748b' }}>{text || 'N/A'}</Text>
    },
    {
    ...
    {
      title: <Text style={{ fontSize: '12px', color: '#64748b', fontWeight: 600 }}>NGÀY GD</Text>,
      dataIndex: 'transactionDate',
      key: 'transactionDate',
      width: 120,
      sorter: (a, b) => new Date(a.transactionDate || 0) - new Date(b.transactionDate || 0),
      render: (date) => {
        ...
        return (
          <Text style={{ color: '#475569', whiteSpace: 'nowrap' }}>{timeStr} {dateStr}</Text>
        );
      }
    },
  ];
